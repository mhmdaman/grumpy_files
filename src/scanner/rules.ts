...
import { DEFAULT_BUNDLE_EXTENSIONS } from './categories';
...
export const DEFAULT_CONFIG: ScanConfig = {
  ...
  includeHidden: true,
  bundleExtensions: DEFAULT_BUNDLE_EXTENSIONS,
};
 ...
 */
export function isLargeFile(sizeBytes: number, config: ScanConfig): SizeLabel | null {
  if (sizeBytes > config.veryLargeBytes) return 'Very Large';
  if (sizeBytes > config.largeBytes) return 'Large';
  if (sizeBytes > config.mediumBytes) return 'Medium';
  return null;
}
...
export const BUILD_AND_DEPENDENCY_DIR_NAMES = new Set([
  'node_modules',
  'publish',
  'bin',
  'obj',
...
export const NON_USER_EXTENSIONS = new Set([
  'dll',
  'pdb',
  'dylib',
  'so',
 ...
 */
export function isBuildOrDependencyPath(filePath: string): boolean {
  const normalized = filePath.replace(/\\/g, '/');
  const parts = normalized.split('/');
  for (const part of parts) {
    const lower = part.toLowerCase();
    // Match exact directory names or common build prefixes like "publish 2"
    if (BUILD_AND_DEPENDENCY_DIR_NAMES.has(lower)) return true;
    if (lower.startsWith('publish') || lower.startsWith('build')) return true;
  }
  return false;
}
 ...
 */
export function isUserFacingFile(extension: string, category: string): boolean {
  const ext = extension.toLowerCase();
  if (NON_USER_EXTENSIONS.has(ext)) return false;
  return true;
}
...
export function isOldFile(
  ...
  if (ageDays >= config.veryOldDays) return 'Very Old';
  if (ageDays >= config.oldDays) return 'Old';
  return null;
}
